...
public class ProverHelper {
  ...
  static boolean infFromAutomaton(String automatonName, Automaton M) {
   ...
   */
  static boolean testCommand(String testName, int needed) {
  ...
  static List<String> findAccepted(Automaton M, String testName, int needed) {
    // we don't want to count multiple representations of the same value as distinct accepted values
    M.randomLabel();
    M = AutomatonLogicalOps.removeLeadingZeroes(M, M.getLabel(), false, null, null);

    boolean infinite = infFromAutomaton(testName, M);
